...
const ChapterIdpage = async ({
  ...
  const {chapter , attachments , course , muxData , isPurchased , nextChapter , userProgress } = await getChapter({userId , chapterId , courseId})
                      ...
                      <div className="p-4 space-y-4 ">
                        {attachments.map((attachment) => (
                            <a href = {attachment.url} 
                              target = '_blank'
                            key  ={attachment.id}    className = " rounded-md hover:underline flex gap-2 items-center p-3 w-full   bg-sky-200" >
                                <File />
